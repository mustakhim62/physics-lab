/*	Scrollbar[] scr = new Scrollbar[2];
    Thread workThread = null; // переменная типа Поток
    Thread T_f_nuThr = null;*/
var imHeight;
var imWidth;
...
function init() {
    //		setBackground(new Color(222,214,198));
    //		OS.offscreenGC.setFont(new Font("Serif",Font.BOLD,13));
...
function mousePressed(e) {
    ...
    //console.log("mouse.x>imX+dx= "+(mouse.x>imX+dx)+" mouse.x<imX+dx+imWidth= "+(mouse.x<imX+dx+imWidth)+" imX= "+imX+" imWidth= "+imWidth+" dx= "+dx+" bBeginDrag= "+bBeginDrag)
    if (mouse.x > imX1 + dxC && mouse.x < imX1 + dxC + 25 && mouse.y > 4 && mouse.y < 4 + 15) {
        bDragCrawl = true;
        dxC = imX1 - mouse.x + dxC;
    } else bDragCrawl = false;
    //console.log("mouse.x>imX1+dxC= "+(mouse.x>imX1+dxC)+" mouse.x<imX1+dxC+25= "+(mouse.x<imX1+dxC+25)+" imX1= "+imX1+" mouse.x= "+mouse.x+" mouse.y= "+mouse.y+" dxC= "+dxC+" bDragCrawl= "+bDragCrawl)
} //mouseDown
// ============================================
...
function mouseReleased(e) {
    ...
    dx = 0;
    dxC = 0;
    //alert("bBeginDrag= "+bBeginDrag+" bDragCrawl= "+bDragCrawl)	
}
// ============================================  
// keyPressed
// ============================================
...
function paint() {
    ...
    // rcImage = new Rectangle(imX+dx, imY0, imWidth, imHeight);
    if (OnOff) {
        context.fillStyle = "rgb(222,214,198)";
