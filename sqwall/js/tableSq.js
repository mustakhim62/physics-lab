...
var nspace = 2; //3;
var cells = new Array();
var labels = ["n", "E/U" + String.fromCharCode(959)];
...
function prepare_screenT(cont, nrows, ncols) {
    ...
    for (i = 1; i < ncols; i++) {
        ...
        for (j = 0; j < nrows; j++) {
            ...
            //		cells[j][i]=new MyCell(cont,String(j)+""+String(i),"rgb(248,248,248)",xint+1, yint+1,w_col-2, h_row-2,k);
            cells[j][i].drawCell();
...
function setValueTable(i, S) {
    nc = Math.floor(i / 2) + 1;
    nr = i % 2;
    //	nr=i%2-1;if(nr<0)nr=0;
    //console.log("i= "+i+" nc= "+nc+" nr= "+nr+" S= "+S)
    S = round_double(S);
